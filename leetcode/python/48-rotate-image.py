...
class Solution:
    ...
    def rotate_2(self, matrix: List[List[int]]) -> None:
        ...
        Do not return anything, modify matrix in-place instead.
        """
        self.transpose_n_by_n(matrix)
        n=len(matrix)
        for i in range(n):
            # matrix[i].reverse()
            self.my_reverse(matrix[i])
            
    
    def my_reverse(self, arr):
        n = len(arr)
        for i in range(n//2):
            arr[i], arr[n-1-i] = arr[n-1-i], arr[i]
    
    def transpose_n_by_n(self, matrix):
        """ in-place tranpose a n by n matrix"""
        n=len(matrix)
